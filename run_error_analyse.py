...
"""Run Error analysis against ELECTRA model on a downstream task."""
...
def model_fn_builder(config: configure_finetuning.FinetuningConfig, tasks,
  ...
  def model_fn(features, labels, mode, params):
    ...
    if pretraining_config is not None:
      ...
      utils.log("[INFO]: Using checkpoint", init_checkpoint)
...
def run_analyse(config: configure_finetuning.FinetuningConfig, pretraining_config):
  """Run analysis."""
  ...
  # Evaluate num_trials models with different random seeds
...
def main():
  ...
  pretraining_config = {
    'model_dir': args.init_from_checkpoint
  }

  run_analyse(configure_finetuning.FinetuningConfig(
      args.model_name, args.data_dir, **hparams), pretraining_config)
